...
public class AesEncryption : IAesEncryption
{
    ...
    public AesEncryption(IOptionsSnapshot<AesEncryptionModel> aesEncryptionModel)
    {
        _encryptionKey = aesEncryptionModel.Value.Key;
    }
    ...
    public string Encrypt(string plainText)
    {
        if (string.IsNullOrEmpty(plainText))
            throw new BadRequestException(nameof(plainText));

        // Derive a new Salt and IV from the Key
        using var keyDerivationFunction = new Rfc2898DeriveBytes(_encryptionKey, _saltSize);
        ...
        using var encryptor = Aes.Create().CreateEncryptor(keyBytes, ivBytes);
    ...
    public string Decrypt(string ciphertext)
    {
        if (string.IsNullOrEmpty(ciphertext))
            throw new BadRequestException(nameof(ciphertext));

        // Extract the salt from our ciphertext
        var allTheBytes = Convert.FromBase64String(ciphertext);
        var saltBytes = allTheBytes
            .Take(_saltSize)
            .ToArray();

        var ciphertextBytes = allTheBytes
            .Skip(_saltSize)
            .Take(allTheBytes.Length - _saltSize)
            .ToArray();

        using var keyDerivationFunction = new Rfc2898DeriveBytes(_encryptionKey, saltBytes);
        ...
        using var decryptor = Aes.Create().CreateDecryptor(keyBytes, ivBytes);
